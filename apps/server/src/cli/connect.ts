...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `t3 connect link` to authorize and enable Ark Link."
    : !status.desired
      ? "Run `t3 connect link` to enable Ark Link."
      : !status.linked
        ? "Start Ark to provision the environment link and launch its managed tunnel."
        : undefined;
...
const confirmRelayClientInstall = (version: string) =>
  Prompt.run(
    Prompt.confirm({
      message: `The Ark relay client is required for Ark Link. Download and install version ${version}?`,
      initial: false,
    }),
  );
...
const logCloudDisconnectFailure = (
  ...
  Effect.logWarning("Ark Link disconnect operation failed.").pipe(
    Effect.annotateLogs({
      operation,
      clearAuthorization,
...
export const reportCloudDisconnectResults = Effect.fn("cloud.cli.report_disconnect_results")(
  function* (input: {
    ...
    if (input.liveResult.status === "failed") {
      ...
      yield* Console.warn(
        "Ark Link is disabled, but the running server could not stop its tunnel.\nRestart that server to stop the connector.",
      );
    } else {
      yield* Console.log("Ark Link is disabled locally.");
    }
...
const disconnectCloud = Effect.fn("cloud.cli.disconnect")(function* (options: {
  ...
  if (options.clearAuthorization) {
    yield* Console.log("Signed out of Ark Link locally.");
  }
});
...
const connectLoginCommand = Command.make("login", {
  ...
  Command.withDescription("Authorize the Ark Link CLI without enabling remote access."),
  Command.withHandler((flags) =>
    runCloudCommand(
      ...
      Effect.gen(function* () {
        const tokens = yield* CliTokenManager.CloudCliTokenManager;
        yield* tokens.get;
        yield* Console.log("Signed in to Ark Link.");
      }),
    ),
  ),
);
...
const connectLinkCommand = Command.make("link", {
  ...
  Command.withDescription("Authorize this environment for Ark Link on next start."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        if (Option.isNone(installed)) {
          yield* Console.log("Ark Link setup cancelled. The relay client was not installed.");
          return;
        }
        yield* Console.log(
        ...
        yield* Console.log(
          "This Ark environment will be available through Ark Link the next time Ark starts.",
        );
      }),
    ),
  ),
...
const connectStatusCommand = Command.make("status", {
  ...
  Command.withDescription("Show persisted Ark Link and relay client state."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
...
const connectUnlinkCommand = Command.make("unlink", {
  ...
  Command.withDescription("Disable Ark Link while retaining the stored authorization."),
  Command.withHandler((flags) =>
...
const connectLogoutCommand = Command.make("logout", {
  ...
  Command.withDescription("Disable Ark Link and clear the stored CLI authorization."),
  Command.withHandler((flags) =>
...
export const connectCommand = Command.make("connect").pipe(
  Command.withDescription("Manage headless Ark Link access."),
